...
void LAppModel::Update()
{
    ...
    double prevX = _model->GetParameterValue(_idParamAngleX);
    double prevY = _model->GetParameterValue(_idParamAngleY);
    double prevZ = _model->GetParameterValue(_idParamAngleZ);
    ...
    const double targetX = _face->GetY();
    const double targetY = _face->GetX() * (-1);
    const double targetZ = _face->GetZ() * (-1);
    const double targetV = _face->GetVolume();

    double vx = targetX - prevX;
    double vy = targetY - prevY;
    double vz = targetZ - prevZ;
    double length = std::sqrt(vx * vx + vy * vy + vz * vz) + 0.00001;
    double dx = vx / length;
    double dy = vy / length;
    double dz = vz / length;
    double speed = std::max(0.5, length * 0.5);
    dx *= speed; 
    dy *= speed; 
    dz *= speed; 
   
    double volume = std::min(1.0, targetV / 1000);

    _model->SetParameterValue(_idParamAngleX, prevX + dx);
    _model->SetParameterValue(_idParamAngleY, prevY + dy);
    _model->SetParameterValue(_idParamAngleZ, prevZ + dz);
    ...
    {
        _model->AddParameterValue(_lipSyncIds[i], volume, 0.8f);
    }
